...
export default function App() {
  ...
  const handleSend = async () => {
    ...
    await fetch(`${API}/api/messages`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ from: candidateName, text: msgInput.trim() }) });
  ...
  const handlePost = async () => {
    ...
    await fetch(`${API}/api/announcements`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: newAnn.trim() }) });
    setNewAnn(""); fetchState();
  };

  const handleReply = async (id) => {
    const txt = orgReply[id]; if (!txt?.trim()) return;
    await fetch(`${API}/api/messages/${id}/reply`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: txt.trim() }) });
    setOrgReply(p => ({ ...p, [id]: "" })); fetchState();
  };

  const handleUpvote = async (id) => {
    await fetch(`${API}/api/messages/${id}/upvote`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: candidateName }) });
    fetchState();
  };

  const handlePatchAnnouncement = async (id, patch) => {
    await fetch(`${API}/api/announcements/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(patch) });
    fetchState();
  };

  const saveEditPin = async () => {
    if (!editPinText.trim()) return;
    await handlePatchAnnouncement(editingPinId, { text: editPinText.trim() });
                ...
                <div key={a.id} className={`ann-row${a.pinned ? "" : " plain"}`}>
                  <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4 }}>
                    ...
                    {a.pinned && editingPinId !== a.id && (
                      <>
                        <button className="btn-xs" onClick={() => { setEditingPinId(a.id); setEditPinText(a.text); }}>Edit</button>
                        <button className="btn-xs" onClick={() => handlePatchAnnouncement(a.id, { pinned: false })}>Unpin</button>
                      </>
                    )}
                    {!a.pinned && (
                      <button className="btn-xs" onClick={() => handlePatchAnnouncement(a.id, { pinned: true })}>Pin</button>
                    )}
                  </div>
                  {editingPinId === a.id ? (
                    <div>
